        ...
        for (div <- 1.0 to 128.0 by 0.1) {
          if (clocks._1 * mult >= 600.0) {
            ...
            if (outClock == clocks._2) {
              return apply(clk, reset, roundAt(3)(1000.0/clocks._1), mult.toDouble, div.toDouble)
            } else {
              val dif = outClock - clocks._2
              if (abs(dif.toDouble) < closest._1) {
                closest = (abs(dif.toDouble), roundAt(3)(mult.toDouble), roundAt(3)(div.toDouble))
              }
            }
          }
        }
